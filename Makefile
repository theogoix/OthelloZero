...
BINDINGS_LINK_OBJECTS = $(OTHELLO_OBJ) $(ENGINE_OBJ)
BINDINGS_BUILD_FLAGS = $(PYBIND11_INCLUDES) $(LIBTORCH_INCLUDE)
...
$(BINDINGS_SHARED_DIR)/%$(PYTHON_SUFFIX): $(BINDINGS_BUILD_DIR)/%.o $(BINDINGS_LINK_OBJECTS)
	@mkdir -p $(dir $@)
	#@echo $^
	$(CXX) $(CXXFLAGS) -shared -o $@ $^ $(BINDINGS_SHARED_FLAGS) $(LIBTORCH_LIB_DIR) $(LIBTORCH_LIBS)
